...
| #135 iPad launch crash | AI crash summary received; Medium binary/initializer audit active | Determine whether chooser appears and inspect matching Exception/Termination/backtrace before attributing cause; no repeat installation | A10X/iPadOS report; declared minOS alone is not device acceptance |
...
| Android preview2/code29 (local, in progress) | Clean fdda4c1 source, alarm guard correction; excludes unmerged scalar optimization | Medium owns isolated build/audits; no artifact ready, no device install or publication yet |
| iPhone/iPad 0.4.13/build 29 (intended) | Separate explicitly authorized manual release task owns build, merge, publication and README; no completed candidate declared here | Manual task failed three turns with the same connection timeout; automatic identical retries stopped. Resume that existing task after the connection recovers. Await exact artifact/source/audit handoff; scheduled coordinator never publishes IPA |
