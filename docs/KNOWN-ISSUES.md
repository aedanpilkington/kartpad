...
| [#123](https://github.com/chrissotraidis/kartpad/issues/123) Pixel online-menu freezes | Pixel 8 Pro / Android 17 API 37 / Mali-G715. The reporter now confirms online-menu freezes at 1x Native / Original 4:3 with Renderer Validation off. Audio stalls and catches up while the native three-dot menu stays responsive; offline play/races work. This supersedes the earlier archive setting mismatch. A newer reported0.4.13 console excerpt shows slowdown during/after prewarm; FPS/CPU/phase windows differ and selected logs contain no network tags or health context. Existing-file network tags and three or four matching health samples are requested, with no new run. See the [timing review](artifacts/2026-09-09/pixel-online-log-review.md). Investigation targets synchronous guest/network waits; neither a network nor renderer root cause is established. |
