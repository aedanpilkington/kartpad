...
| [#123](https://github.com/chrissotraidis/kartpad/issues/123) Pixel online-menu freezes | Pixel 8 Pro / Android 17 API 37 / Mali-G715. Reporter distinguishes online menus from working offline play/races. Newest code-23 archive session records configured 4x and validation on, while the report says 1x; older sessions are mixed in. Exit entries are user-requested/self exits, with no matching build attribution. A fresh 1x/4:3 validation-off offline/online-menu comparison is requested. No network or renderer root cause is established. |
