...
class TvOSContractTests(unittest.TestCase):
    ...
    def test_release_contracts_cover_ios_and_tvos(self):
        ...
        for script in (ios_package, ios_audit):
            self.assertIn('RELEASE_TAG = "v0.4.13-ios.1"', script)
            self.assertIn('APP_VERSION = "0.4.13"', script)
        ...
        self.assertIn('APP_BUILD = "29"', ios_package)
        self.assertIn('APP_BUILD = "29"', ios_audit)
